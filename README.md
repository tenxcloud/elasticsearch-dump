...
(local)
```bash
npm install elasticdump-tenx
./node_modules/.bin/elasticdump
```

(global)
```bash
npm install elasticdump-tenx -g
elasticdump
```
...
--awsIniFileName
                    ...
                    (default: config)
--ignoreUnavailable
                     This setting must be either True or False. When the
                     ignoreUnavailable option is False and an index is
                     missing the snapshot request will fail. The default
